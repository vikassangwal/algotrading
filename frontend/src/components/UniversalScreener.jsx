...
const DEFAULT_OPPORTUNITIES = [
  ...
  { symbol: "RVNL", segment: "EQUITY", current_price: 580.00, decision: "BUY", analytical_score: 0.85, catalysts: "Rail Infra Orderbook, Intraday VWAP Breakout", tp: 650.00, sl: 535.00, strategy: ["SCALPING", "INTRADAY", "BREAKOUT", "TOP_PROFIT"] },
  { symbol: "IREDA", segment: "EQUITY", current_price: 235.40, decision: "BUY", analytical_score: 0.84, catalysts: "Green Financing Expansion, Intraday Scalping Momentum", tp: 275.00, sl: 215.00, strategy: ["SCALPING", "INTRADAY", "TOP_PROFIT"] },
  { symbol: "SUZLON", segment: "EQUITY", current_price: 68.40, decision: "BUY", analytical_score: 0.76, catalysts: "Clean Energy Volume Spike, Intraday Momentum", tp: 82.00, sl: 61.50, strategy: ["SCALPING", "INTRADAY", "TOP_PROFIT"] },
  ...
  { symbol: "GOLD (GC=F)", segment: "COMMODITY", current_price: 2420.50, decision: "STRONG BUY", analytical_score: 0.91, catalysts: "Central Bank Buying, Intraday Macro Breakout", tp: 2520.00, sl: 2360.00, strategy: ["SCALPING", "INTRADAY", "BREAKOUT", "TOP_PROFIT"] },
  { symbol: "SILVER (SI=F)", segment: "COMMODITY", current_price: 28.40, decision: "BUY", analytical_score: 0.74, catalysts: "Industrial Demand Spike, Intraday Momentum", tp: 31.50, sl: 26.80, strategy: ["SCALPING", "INTRADAY"] },
  ...
  { symbol: "USDINR (INR=X)", segment: "CURRENCY", current_price: 83.92, decision: "HOLD", analytical_score: 0.10, catalysts: "RBI Range Defense, Rangebound Scalping", tp: 84.20, sl: 83.60, strategy: ["SCALPING", "INTRADAY"] },
...
export default function UniversalScreener({ token, globalSymbol, onSelectSymbol, initialFilter = 'ALL' }) {
  ...
  let filteredData = data.filter(d => {
    ...
    if (filter === '⚡ SCALPING (1-5 MIN)') return (d.strategy && d.strategy.includes('SCALPING')) || (d.catalysts && (d.catalysts.toLowerCase().includes('scalping') || d.catalysts.toLowerCase().includes('momentum') || d.catalysts.toLowerCase().includes('spurt')));
    ...
    <div style={{ padding: '20px', color: 'var(--text-primary)' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
        <div>
          ...
          <p style={{ margin: 0, color: 'var(--text-secondary)', fontSize: '0.9rem' }}>
            Filter dynamically by Scalping (1-5m), Intraday, Swing Trading, Breakout, Options F&O, Nifty 50, Sensex & Commodities.
          </p>
        </div>
      ...
      <div style={{ display: 'flex', gap: '10px', marginBottom: '20px', backgroundColor: '#1e293b', padding: '12px', borderRadius: '8px', flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        {[
          ...
          const isScalp = f.includes('SCALPING');
          const isStrategy = isScalp || f.includes('INTRADAY') || f.includes('SWING') || f.includes('BREAKOUT') || f.includes('OPTIONS');
          ...
          return (
            <button
              ...
              style={{
                ...
                backgroundColor: isActive ? (isTop ? '#f59e0b' : isScalp ? '#ef4444' : isStrategy ? '#10b981' : '#8b5cf6') : (isTop ? 'rgba(245, 158, 11, 0.15)' : isScalp ? 'rgba(239, 68, 68, 0.15)' : isStrategy ? 'rgba(16, 185, 129, 0.12)' : 'transparent'),
                color: isActive ? (isTop ? '#0f172a' : 'white') : (isTop ? '#fbbf24' : isScalp ? '#f87171' : isStrategy ? '#34d399' : '#cbd5e1'),
                border: '1px solid',
                borderColor: isActive ? (isTop ? '#f59e0b' : isScalp ? '#ef4444' : isStrategy ? '#10b981' : '#8b5cf6') : (isTop ? '#f59e0b' : isScalp ? '#ef4444' : isStrategy ? '#10b981' : '#334155'),
                borderRadius: '20px',
                cursor: 'pointer',
                fontSize: '0.85rem',
                fontWeight: 'bold',
                boxShadow: (isActive && isTop) ? '0 0 10px rgba(245, 158, 11, 0.4)' : (isActive && isScalp) ? '0 0 10px rgba(239, 68, 68, 0.4)' : 'none'
              }}
            >
              {f}
            </button>
